...
export default function Transfer({ onBack, userData }: TransferProps) {
  ...
  const [bank, setBank] = useState('nova_b');
  ...
  const myAccount = userData?.accounts?.[0]?.account_number || 'B1001';
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      {/* Header - Blue theme */}
      <div className="bg-[#1a237e] text-white p-4 flex items-center shadow-md z-10">
        <button onClick={step === 1 ? onBack : () => setStep(step - 1)} className="p-2 -ml-2 hover:bg-indigo-800 rounded-full transition-colors">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-lg font-medium ml-2">Chuyển tiền {bank === 'nova_a' ? '(2PC Liên ngân hàng)' : ''}</h1>
      </div>
          ...
          {[1, 2, 3].map((s) => (
            <div 
              ...
              className={`flex-1 py-3 text-center text-sm font-medium relative ${
                step === s ? 'text-[#1a237e]' : 'text-gray-400'
              }`}
            >
              {s === 1 ? 'Nhập thông tin' : s === 2 ? 'Xác nhận' : 'Kết quả'}
              {step === s && (
                <div className="absolute bottom-0 left-0 w-full h-0.5 bg-[#1a237e]"></div>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="p-4">
          {step === 1 && (
            <form onSubmit={handleNext} className="space-y-5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">NovaBank B - PostgreSQL</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-[#1a237e]">{formatCurrency(myBalance)}</p>
                  </div>
                </div>
              </div>
                ...
                <div>
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="block w-full pl-10 pr-10 py-3 border border-gray-200 rounded-xl focus:ring-[#1a237e] focus:border-[#1a237e] text-sm appearance-none bg-gray-50 outline-none font-medium text-gray-800"
                    >
                      <option value="nova_b">🏦 NovaBank B (Nội bộ - PostgreSQL)</option>
                      <option value="nova_a">🏦 NovaBank A (Liên ngân hàng - SQL Server)</option>
                    </select>
                  </div>
                  {bank === 'nova_a' && (
                ...
                <div>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="block w-full pl-10 pr-3 py-3 border border-gray-200 rounded-xl focus:ring-[#1a237e] focus:border-[#1a237e] text-sm bg-gray-50 outline-none font-medium text-gray-800"
                      placeholder={bank === 'nova_a' ? 'Nhập số TK Bank A (vd: A1001)' : 'Nhập số TK Bank B (vd: B1002)'}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-4">
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="block w-full pl-4 pr-12 py-3 text-lg font-bold border border-gray-200 rounded-xl focus:ring-[#1a237e] focus:border-[#1a237e] bg-gray-50 outline-none text-[#1a237e]"
                      placeholder="0.00"
                    />
                  ...
                  <div className="mt-3 flex overflow-x-auto pb-2 gap-2 scrollbar-hide">
                    {[50, 100, 200, 500].map(val => (
                      <button
                        ...
                        className="px-4 py-1.5 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-full border border-indigo-100 whitespace-nowrap active:bg-indigo-100"
                      >
                        {val} USD
                      </button>
                    ))}
                ...
                <div>
                  <input
                    ...
                    className="block w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-[#1a237e] focus:border-[#1a237e] text-sm bg-gray-50 outline-none"
              ...
              <div className="pt-4 pb-8">
                <button
                  type="submit"
                  className="w-full flex justify-center items-center py-3.5 px-4 rounded-xl shadow-md text-base font-semibold text-white bg-[#1a237e] hover:bg-indigo-900 active:scale-[0.98] transition-all"
                >
                  Tiếp tục
                </button>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 text-center">
                ...
                <h2 className="text-3xl font-bold text-[#1a237e]">{formatCurrency(Number(amount))}</h2>
              </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
                ...
                <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                  ...
                  <span className="font-semibold text-gray-900">{bank === 'nova_b' ? 'NovaBank B' : 'NovaBank A'}</span>
                </div>
                <div className="flex justify-between items-center pb-3 border-b border-gray-100">
                  ...
                  <span className={`font-semibold px-2 py-0.5 rounded-full text-xs ${
                    ...
                    {bank === 'nova_a' ? '2PC Liên ngân hàng' : 'Nội bộ'}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-indigo-600">Miễn phí</span>
                </div>
              </div>
              ...
              <div className="pt-4 pb-8">
                <button
                  ...
                  className="w-full flex justify-center items-center py-3.5 px-4 rounded-xl shadow-md text-base font-semibold text-white bg-[#1a237e] hover:bg-indigo-900 active:scale-[0.98] transition-all disabled:opacity-70"
                >
                  {isProcessing ? (
                    <div className="flex items-center space-x-2">
                      <Loader2 className="w-5 h-5 animate-spin" />
                      <span>{bank === 'nova_a' ? 'Đang xử lý 2PC...' : 'Đang chuyển...'}</span>
                    </div>
                  ) : (
                    'Xác nhận chuyển'
                  )}
          ...
          {step === 3 && (
            <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 text-center mt-4">
              <div className={`w-20 h-20 ${isSuccess ? 'bg-indigo-100' : 'bg-red-100'} rounded-full flex items-center justify-center mx-auto mb-6`}>
                {isSuccess ? (
              ...
              <p className={`text-3xl font-bold ${isSuccess ? 'text-[#1a237e]' : 'text-red-600'} mb-4`}>
              ...
              <div className="bg-gray-50 rounded-xl p-4 text-left space-y-3 mb-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">{bank === 'nova_a' ? '2PC' : 'Internal'}</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              <div className="space-y-3">
                <button
                  onClick={resetForm}
                  className="w-full py-3.5 px-4 rounded-xl shadow-sm text-base font-semibold text-white bg-[#1a237e] active:scale-[0.98] transition-all"
                >
                  Giao dịch mới
                </button>
                <button
                  onClick={onBack}
                  className="w-full py-3.5 px-4 rounded-xl text-base font-semibold text-[#1a237e] bg-indigo-50 active:bg-indigo-100 transition-all"
                >
                  Về trang chủ
                </button>
              </div>
